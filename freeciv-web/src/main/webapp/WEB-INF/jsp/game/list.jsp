			...
			$
					.getJSON(
							...
							function(data) {
								...
								$($($(".nav-tabs").children()[2]).children()[0])
										.html(
												"Play-By-Email (" + data.length
														+ ")");
		...
		<div>
			<ul class="nav nav-tabs hidden-xs" role="tablist">
				<li role="presentation"
					class="${view == 'singleplayer' or empty view ? 'active' : ''}"><a
					href="#single-player-tab" aria-controls="single-player" role="tab"
					data-toggle="tab">Single-player (${singlePlayerGames})</a></li>
				<li role="presentation"
					class="${view == 'multiplayer' ? 'active' : ''}"><a
					href="#multi-player-tab" aria-controls="multi-player" role="tab"
					data-toggle="tab">Multiplayer (${multiPlayerGames})</a></li>
				<li role="presentation"
					class="${view == 'play-by-email' ? 'active' : ''}"><a
					href="#play-by-email-tab" aria-controls="play-by-email" role="tab"
					data-toggle="tab">Play-By-Email</a></li>
			</ul>
			<ul class="nav nav-tabs hidden-lg hidden-md hidden-sm" role="tablist">
				<li role="presentation"
					class="${view == 'singleplayer' or empty view ? 'active' : ''}"><a
					href="#single-player-tab" aria-controls="single-player" role="tab"
					data-toggle="tab">Single (${singlePlayerGames})</a></li>
				<li role="presentation"
					class="${view == 'multiplayer' ? 'active' : ''}"><a
					href="#multi-player-tab" aria-controls="multi-player" role="tab"
					data-toggle="tab">Multi (${multiPlayerGames})</a></li>
				<li role="presentation"
					class="${view == 'play-by-email' ? 'active' : ''}"><a
					href="#play-by-email-tab" aria-controls="play-by-email" role="tab"
					data-toggle="tab">Play-By-Email</a></li>
			...
			<div class="tab-content">
				<div role="tabpanel"
					class="tab-pane ${view == 'singleplayer' or empty view ? 'active' : ''}"
						...
						<table id="singleplayer-table" class="table">
							...
							<c:forEach items="${singlePlayerGameList}" var="game">
								<tr class="${game.isProtected() ? '.private-game' : '' }">
									<td><c:if test="${game.flag ne 'none'}">
											<img src="/images/flags/${game.flag}-web.png"
												alt="${game.flag}" width="80" height="50"
												title="${game.turn}">
										</c:if></td>
									<td class="hidden-xs"><a
										href="/data/savegames/map-${game.port}.map.gif"> <img
											src="/data/savegames/map-${game.port}.map.gif" width="80"
											height="50">
									</a></td>
									...
									<td><a class="label label-success label-lg"
										href="/webclient?action=observe&amp;civserverport=${game.port}&amp;civserverhost=${game.host}"
										title="Observe"> 2D</a> <a
										class="label label-success label-lg"
										href="/webclient?renderer=webgl&amp;action=observe&amp;civserverport=${game.port}&amp;civserverhost=${game.host}"
										title="Observe"> 3D</a> <a
				...
				<div role="tabpanel"
					class="tab-pane ${view == 'multiplayer' ? 'active' : ''}"
					id="multi-player-tab">
							...
							<c:forEach items="${multiPlayerGamesList}" var="game">
								<tr
									...
									<td><c:choose>
										...
										</c:choose> <a class="label label-primary label-lg"
										href="/game/details?host=${game.host}&amp;port=${game.port}">
											Info </a></td>
								</tr>
							</c:forEach>
				...
				<div role="tabpanel"
					class="tab-pane ${view == 'play-by-email' ? 'active' : ''}"
					id="play-by-email-tab">
					<div class="row">
						<div class="col-md-12">
							<p>A Play-By-Email game is a deathmatch on a small map with
								up to 4 human players, playing with alternating turns, and
								players get an e-mail every time it is their turn to play. These
								games are often played over a long time period, each player has
								7 days to complete their turn.</p>
							<p>
								To start a new Play-By-Email game, <u></u>
							</ul>
							<a href="/webclient/?action=pbem">log in here</a></u>. To play your
							turn in a running Play-By-Email game, click on the link in the
							last e-mail you got from Freeciv-web. Games are expired after 7
							days if you don't play your turn.
							</p>
						</div>
					</div>
